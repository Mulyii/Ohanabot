...
def Lolicon(N:int = 1, Tag:str = "", R18:int = 0):
    ...
    msg = ""
    if 1 <= N <= 10:
        msg += f"Bot_NICKNAME为你准备了{N}张随机{'r18'if R18 else ''}{tag[5:]}。"
    elif N > 10:
        N = 1
        if R18:
            msg += f"Bot_NICKNAME为你的身体着想，为你准备了一张随机r18{tag[5:]}色图。"
        else:
            msg += f"Bot_NICKNAME被禁止单次发送超过10张色图...但是，Bot_NICKNAME为你准备了一张随机{tag[5:]}。"
    else:
        N = 1
        msg += f"Bot_NICKNAME接收到了奇怪的数量参数，不过Bot_NICKNAME送你一张随机{'r18'if R18 else ''}{tag[5:]}。"
